...
def get_bottleneck_values(sess, image_lists, image_dir, bottleneck_dir,
...
def get_bottleneck_list(image_dir,label_name):
    ...
    image_lists = create_image_lists('/usa/psu/Documents/CISC849/project/example/flower_photos', 10,10)
    class_count = len(image_lists.keys())
    if class_count == 0:
        ...
        with tf.Session(graph=graph) as sess:
            ...
            for subf_i in os.listdir(image_dir):
                ...
                for file_i in os.listdir(image_dir+subf_i):
                    ...
                    bottleneck_value=get_bottleneck_values(sess, image_lists, image_dir+subf_i+'/'+file_i,
                                      '', jpeg_data_tensor,
                                      decoded_image_tensor, resized_image_tensor,
                                      bottleneck_tensor, 'https://tfhub.dev/google/imagenet/inception_v3/feature_vector/1')
...
def iter_dataset(file_path,model,batch_size):
    ...
    label_name=os.listdir(file_path+'train/')
    training_data=[[],[]]
    file_num=0
    bottlenect_vector,label_vector=get_bottleneck_list(final_path,label_name)

    for index_i in range(len(bottlenect_vector)):

        training_data[0].append(bottlenect_vector[index_i])
        training_data[1].append(label_vector[index_i])
...
if __name__ == '__main__':
    get_bottleneck_list('/usa/psu/Documents/CISC849/project/example/flower_photos/roses/12240303_80d87f77a3_n.jpg')